...
def choisirCaseVide(grille):
    cases_vides = [(i, j) for i in range(len(grille)) for j in range(len(grille)) if grille[i][j] == '-']
...
n = 4
grids = parse_grids_from_file(file_path, n)
...
print("Winrate  = " + str((nbwin / nbline) * 100) + " %")
print("Loserate = " + str((nblose / nbline) * 100) + " %")
print("Drawrate = " + str((nbdraw / nbline) * 100) + " %")
print("AVG duration = " + str(total_duration / nbline) + " s")
print("Full duration = " + str(total_duration) + " s")
